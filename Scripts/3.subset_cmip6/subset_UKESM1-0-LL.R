...
lat_res <- 1.250000  # Latitude and longitude resolution based on the model grid

##############################################################

m=15  # Setting the model index (11th model)
  ...
  for (v in 1){
    ...
    if(ssp == "historical") {
      ...
      for (d in 1:length(dates)){
        ...
        pixels = rep(NA, 12410)  # Initialize an array to store pixel data
        #p=49
        ...
        for (p in 1:length(guide$lon)){
          ...
          if(in_out == 1){  # If inside, process pixel data
            ...
            X <- (guide[p,2]/ lon_res)+0.5  # Calculate X index
            Y <- ((guide[p,3] + 90)/lat_res)+0.5  # Calculate Y index
            
            
            
            pixel <- array[X,Y, 11160:23400]
      ...
      data <- rbind(pixels_d1)  # Combine data for both dates
      ...
      #rownames(data) <- as.character(1:length(data$pixel_1))  # Set row names
    ...
    if(ssp == "ssp245"|ssp == "ssp370"|ssp == "ssp585") {
      ...
      for (d in 1:length(dates)){
        ...
        if (d == 1){
          ...
          pixels = rep(NA, 12775)  # Initialize pixel data
          #p=146
          for (p in 1:length(guide$lon)){
            ...
            #pixel <- tail(pixel,12775)  # Keep last 18250 values
            pixels <- cbind(pixels, pixel_1[-1])  # Append the pixel data
        ...
        if (d == 2){
          ...
          pixels = rep(NA, 18615)  # Initialize pixel data array
          #p=146
          for (p in 1:length(guide$lon)){
            ...
            #pixel <- tail(pixel,18615)  # Keep last 13140 values
            pixels <- cbind(pixels, pixel_1[-1])  # Append the pixel data
