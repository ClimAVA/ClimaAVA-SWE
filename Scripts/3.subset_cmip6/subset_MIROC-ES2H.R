...
lon_res <- 1.406250 ###### CHANGES FOR EVERY MODEL
lat_res <- 1.389030
##############################################################
      ...
      for (d in 1:length(dates)){
        ...
        for (p in 1:length(guide$lon)){
          ...
          if(in_out == 1){
            ...
            Y <- ((guide[p,3] + 90)/lon_res)+1 # Latitude to pixel index conversion
      ...
      for (d in 1:length(dates)){
        ...
        for (p in 1:length(guide$lon)){
          ...
          if (in_out == 1) {
            ...
            Y <- ((guide[p,3] + 90)/lon_res)+1 # X is the pixel position in the netcdf. takes the coordinates of the pixel divide by resolution and sums 90 becouse it is in the north hemisphere 
