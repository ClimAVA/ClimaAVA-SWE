...
lon_res <- 1.125000  ###### CHANGES FOR EVERY MODEL
lat_res <- 1.112091  # Latitude and longitude resolution based on the model grid

##############################################################

m=11  # Setting the model index (11th model)
    ...
    if(ssp == "historical") {
      ...
      for (d in 1:length(dates)){
        ...
        if (d == 1){
          ...
          pixels = rep(NA, 3285)  # Initialize an array to store pixel data
          #p=310
          ...
          for (p in 1:length(guide$lon)){
            ...
            if(in_out == 1){  # If inside, process pixel data
              ...
              Y <- ((guide[p,3] + 90)/lat_res) # Calculate Y index
              
              pixel <- array[X,Y, 1:10220]}  # Extract data for the pixel
            
         
            pixel <- tail(pixel,3285)  # Keep the last 6935 values
        ...
        if (d == 2){
          ...
          pixels = rep(NA, 9125)  # Initialize pixel data array
          #p=200
          for (p in 1:length(guide$lon)){
    ...
    if(ssp == "ssp245"|ssp == "ssp370"|ssp == "ssp585") {
      ...
      for (d in 1:length(dates)){
        ...
        if (d == 1 | d == 2 | d == 3 ){
          ...
          pixels = rep(NA, 8030)  # Initialize pixel data
          #p=146
          for (p in 1:length(guide$lon)){
          ...
          if (d == 1) { pixels_d1 <- pixels[,-1] }  # Store data for the first date
          if (d == 2) { pixels_d2 <- pixels[,-1] }  # Store data for the first date
          if (d == 3) { pixels_d3 <- pixels[,-1] }  # Store data for the first date
        }
        ...
        if (d == 4){
          ...
          pixels = rep(NA, 7300)  # Initialize empty pixel array
          #p=1
          for (p in 1:length(guide$lon)){
            in_out <- guide[p,4]
            ...
            if(in_out == 1){
              ...
              pixel <- array[X,Y, 1:7300]}  # Extract data for the 4th date
      ...
      data <- rbind(pixels_d1, pixels_d2, pixels_d3, pixels_d4)  # Combine data for both dates
